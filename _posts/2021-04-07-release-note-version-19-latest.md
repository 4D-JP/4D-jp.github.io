...
**注記**: 修正により，ソースコード（`/PROJECT/Sources/*`）およびオブジェクトコード（`/PROJECT/DerivedData/CompiledCode/*`）のタイムスタンプが比較されるようになりました。前者の最新が後者の最古よりも新しい場合に限り，ソースコードの再コンパイルが実行されるようになりました。
