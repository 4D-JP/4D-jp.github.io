...
通常，特定バージョンのRリリースで開いたデータベースアプリケーションは，メソッド等をエディターで開いたりしない限り，Rリリースではないバージョンに戻ることができます。しかし，17r5では，ICUライブラリがアップデートされているため，文字・テキスト・オブジェクト型フィールドの再インデックス構築が発生し，元のバージョンに戻ると再々インデックス構築が発生することになります。検証のため，Rリリースと17.xを頻繁に切り替えるつもりであれば，空のデータファイルまたはインデックスが外されたストラクチャを用意しておくと良いでしょう。
