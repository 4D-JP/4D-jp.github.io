...
* ACI0100376 クイックレポートがエディターで開かれた状態で別のレポートを開こうとする場合，開かれたレポートを保存しなければなりませんでした。「保存しますか？」と尋ねるダイアログは表示されますが，選択肢が「OK」しかありません。レポートを開くたびに，不要なレポートがディスクに増えてゆきます。ダイアログを閉じようとしたのであれば「保存しない・キャンセル・保存する」の３択が与えられます。レポートを開こうとした場合にも，同じような動作になるべきです。
