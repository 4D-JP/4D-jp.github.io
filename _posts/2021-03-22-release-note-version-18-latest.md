...
* ACI0101893 `VP PRINT`の出力先をPDFファイルに設定してView Proスプレッドシートを印刷した場合，ピクチャ型を返すフォーミュラが正しく出力されませんでした。PDFエクスポートであれば問題ありません。

* ACI0101885 View Proスプレッドシートの`ROUND`関数が間違っていることがありました。たとえば`=ROUND(18,205;2)`は`18.21`ではなく`18.2`を返します。SpreadJSの不具合です。

**注記**: 修正により，SpreadJS `14.0.8`のパッチがSpreadJS `12.2.5`に統合されました。
