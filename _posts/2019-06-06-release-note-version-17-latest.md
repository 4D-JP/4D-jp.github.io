...
* ACI0099637 ストラクチャエディターのインスペクターに日本語でテーブルまたはフィールドの名前を入力した場合，``return``キーを``3``回（日本語入力の確定・テキスト入力の確定・次のオブジェクトにタブ移動）してからエディターに表示されているストラクチャ図をダブルクリックし，その後，再びインスペクターに日本語でテーブルまたはフィールドの名前を入力しようとすると，日本語の入力ができませんでした。インスペクターのウィンドウを閉じてから再表示させると，日本語入力ができるようになります。

**注記**: タブ移動により，「フォーカス可」「タブ有効」が設定されたチェックボックスがフォーカスを得ている状態で，そのウィンドウを閉じずに別ウィンドウでテキスト入力を開始しようとした場合に問題が発生しました。
