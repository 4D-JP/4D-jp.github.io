...
* ACI0099412 システムロケールが日本語に設定されている場合，4D View Proの日付フォーマットが正しくありませんでした。``vk pattern Full date time``（Windows）は``yyyy'年'M'月'd'日' HH:mm:ss``形式（年月日を含む），``ddd``（Mac）は``M``形式（月を含まない）を返すべきです。
