...
* ACI0105724 コンパイルモードのみ。接続ダイアログのカスタムページでサーバー名とネットワークアドレスを入力しても，「OK」ボタンがアクティブ状態になりませんでした。`tab`キーで入力フィールドから移動すれば，アクティブ状態になります。
...
* ACI0105697 テーブルフォームを印刷した場合，日付型フィールドが出力されませんでした。

* ACI0105776 Windows版のみ。Windows server 2025 24H2で印刷を実行した場合，エラーが返されることがありました。23H2では問題ありません。

* ACI0105675 *Build4D* コンポーネントの`macOSClientArchive`プロパティで自動アップデート用クライアントのパスをテキスト型で指定することができませんでした。`File`型であれば問題ありません。
...
* ACI0105723 プロジェクトが*environment4d.json* を使用している場合，依存関係のダイアログ画面からGitHubパーソナルアクセストークンを編集することができませんでした。
...
* ACI0105727 プロジェクトモードのみ。サブフォームのフォーカスがデフォルトで表示されませんでした。「フォーカスの四角を隠す」プロパティのデフォルト値が間違っているようです。
...
* ACI0105718 `Request`のデフォルト値に空の文字列を渡した場合，OKボタンをクリックすることができませんでした。

* ACI0105707 Mac版のみ。境界線スタイルが「システム」または「くぼみ」に設定されている入力オブジェクトおよびWrite Proエリアは，フォームエディター上で左右の端が`1`ピクセルずれた位置に表示されました。

* ACI0105615 SafariブラウザからWebサーバーに対してDIGEST認証でログインすることができませんでした。Chrome・Firefox・Edgeでは問題ありません。

**注記**: サーバーから返される`WWW-Authenticate`ヘッダーに問題がありました。`CRLF`で区切られた複数のヘッダーが返されるべきですが，改行がエスケープされた単一のヘッダーが返されました。ACI0105039が修正されたことによる副作用のようです。

* ACI0105641 Intelアーキテクチャーのみ。ASTインタープリターが有効にされていないアプリケーションでバリアント型に代入された時間値に対してゼロ除算を実行した場合，アプリケーションがクラッシュしました。
...
* ACI0105703 `var`シンタックスで変数の宣言と初期化を記述し，日付型の変数にプロパティを代入した場合，「引数のタイプが一致していません。」というランタイムエラー`54`が返されました。

```4d
var $date : Date:=This.theDate
```

**注記**: */PROJECT/DerivedData/CompiledCode/classes.json* に出力されたISO日付が文字列から日付にキャストされないことが原因でした。

* ACI0105693 `Progress window set visible`でプログレスバーコンポーネントのウィンドウを非表示にすることができませんでした。

* ACI0105678 Windows版のみ。`100%`以外のDPIが設定されている環境では，ヘッダーの入力オブジェクトにフォーカスがある限り，いつまでも *On Display Detail* が発生しました。

* ACI0105652 デザインモードツールバーの「開く」ボタンをロングクリックした場合，ランタイムエラーが返されました。

* ACI0105662 ステッパーのプロパティリストで「入力可」プロパティを解除した場合，「フォーカス可」プロパティが解除されませんでした。

* ACI0105687 Windows版のみ。詳細が表示された状態でランタイムエラー画面を閉じた場合，ランタイムエラー画面が以降は正常に表示されなくなりました。

* ACI0105669 Windows版のみ。4D Serverをサービス起動した場合，クライアント側の管理画面には「サービス起動」が「いいえ」と表示されました。
...
* ACI0105614 AIKitコンポーネントのドキュメンテーションを開いた場合，デザインモードの反応がとても悪くなりました。
