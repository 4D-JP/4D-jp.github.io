...
* ACI0101264 Mac版のみ。SVGから変換された画像を4D Write Proエリアに挿入した場合，終了時にアプリケーションがクラッシュしました。

**注記**: Write Proによってキャッシュされた画像が解放されるタイミングが遅すぎたようです。SVG画像は，内部的にxercesライブラリのDOMツリーを参照しています。修正により，アプリケーション終了時にライブラリがアンロードされるタイミングが見直されました。

* ACI0101026 Mac版のみ。オブジェクトに``.SF NS Text``フォントが設定されている場合，システムフォントではないフォントが表示されました。

**注記**: 修正により，``.SF NS Text``フォントが設定されているオブジェクトは，フォーミュラエディターのプロパティリストに「システムフォント」と表示され，プロジェクトモード変換では``System Font Regular``という値がエクスポートされるようになりました。
...
* ACI0101219 Windows版のみ。おおきなストラクチャでビルドまたはシンタックスチェックまたはコンパイルを実行した場合，アプリケーションがクラッシュしました。

**注記**: コンパイラーの旧メモリマネージャーに問題がありました。修正により，コンパイラの新メモリマネージャーが採用されることになりました。
...
* ACI0101054 クライアント/サーバー版のみ。プロジェクトモードで``QUERY BY SQL``を実行するとサーバーがフリーズしました。
...
* ACI0100760 4D View Proエリアを表示した直後のタイプ入力に対し，``On After Edit``イベントが発生しませんでした。Macの場合，アプリケーションを再起動した後は問題が発生しません。Windowsの場合，最初に入力したのが数字であれば，問題は発生しません。いずれのプラットフォームでも，最初の文字をフォーミュラバーから入力したのであれば，問題は発生しません。また，セルをダブルクリックして入力モードに移行したのであれば，問題は発生しません。セルを選択した状態から直接タイプ入力を開始した場合に問題が発生します。
