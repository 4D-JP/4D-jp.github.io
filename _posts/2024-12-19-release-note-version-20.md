...
コンパイルモードでは，すべての変数が宣言されており，明示的に宣言されていないイテレーター変数はバリアント型になります。修正前の`For each`構文では，バリアント型に対する代入であっても，すでに代入されている値の型をできるだけ変えないように代入が処理されてしました。修正により，変数が明示的にバリアント型として宣言されていれば，`Null`は`Null`のまま代入されるようになりました。
