...
**注記**: リグレッション（ACI0106449）が発生するため，この修正は撤回されましした。Windowsの場合，改行コードを含むテキストを`EXPORT TEXT`で書き出すことができないのは仕様です。`13`は文字コードではなく，プラットフォーム既定のデリミターを指し，Windowsでは常に`CRLF`となります。
