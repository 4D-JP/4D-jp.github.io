...
* ACI0104872 *Qodly* の標準アクションで作成した新規エンティティのリレーション属性にエンティティを代入した場合，リレーション属性のキーが正しく保存されませんでした。
...
* ACI0104886 中途半端な内容でクラスのソースファイルを保存した場合，プロジェクト再起動後にアプリケーションがクラッシュすることがありました。

* ACI0104810 互いをプロパティとして参照する再帰的な共有クラスをインスタンス化した場合，コンストラクターでエラーが返されました。

**注記**: 共有クラスに限られた問題ではありません。下記のようなコードでも同じ状況が発生します。修正に伴い，共有オブジェクトの実装が見直されました。

```4d
$o1:=New shared object("a"; 123)
$o2:=New shared object("a"; 123)
$o3:=New shared object("o"; $o2)
Use ($o1)
  $o1.o:=$o3
End use 
```

* ACI0104887 コンパイルモードのみ。エラーハンドリングメソッドがインストールされている場合，`Try`式でキャッチしたエラーが`Catch`ブロックで処理されず，エラーダイアログが表示されました。
...
* ACI0104866 延期されたクライアント接続の再開に失敗した場合，内部的なミューテックスが解除されず，クライアントがフリーズする可能性がありました。

* ACI0104777 クライアント/サーバー版のみ。エンティティの時間型属性が正しくサーバーに転送されませんでした。秒数の`1/1000`が値として保存されました。 

* ACI0104877 *Qodly studio* でメソッドまたはクラスを開いた場合，アプリケーションがクラッシュしました。
...
* ACI0104744 デバッガーでメソッドをアボートした場合，ネットワークエラーが*Qodly Studio* に返されました。
