...
* ACI0105952 オブジェクトライブラリの「進捗インジケーター」カテゴリーを選択した場合，何も表示されませんでした。
...
* ACI0105894 `4D.Signal`オブジェクトを共有オブジェクトのプロパティに代入しようとした場合，エラーが返されました。
...
* ACI0106260 `Case`文の条件入力中は，オートコンプリート候補にクラスのプロパティやメンバー関数が表示されませんでした。
...
* ACI0106230「サーバーに接続」画面のウィンドウタイトルに「4D」とだけ表示されました。ACI0097827が修正されたことによる副作用のようです。

* ACI0106252 ビルド版アプリケーションの「サーバー管理」画面のウィンドウタイトルにカスタムアプリケーションの名称ではなく「4D Server」と表示されました。

* ACI0106148 [エイリアス属性](https://developer.4d.com/docs/ja/ORDA/ordaClasses#エイリアス属性-1)をメソッドに記述した場合，デザインモードのコンパイラーを使用すればコンパイルに成功しますが，`Compile project`ではエラーが検出されました。ビルド`100075`から不具合が発生したようです。
...
* ACI0106264 引数を受け取らないメンバー関数の編集中に`$`記号を入力した場合，オートコンプリートの候補リストに当該メンバー関数のローカル変数だけでなく，直後のメンバー関数の命名引数が表示されました。

* ACI0106261 エンティティクラスのオートコンプリート候補リストに`afterSave`イベントが表示されませんでした。他のORDAイベントは表示されます。

* ACI0106222 プロジェクトメソッドの`#DECLARE`行またはメンバー関数の宣言行では「定義に移動」が使用できませんでした。
