...
これが効果的なのは，最初の連鎖ではA2およびA3の評価値が未定義（#VALUE!）であり，未定義を加算した値は未定義に決まっているので，そこで数式の評価がストップし，後続の関数が呼び出されないためです。それぞれのセルは参照先に値が代入されるたび連鎖的に再評価されますが，参照先のセルが未定義である間はそのセルに加算するべき値を求める関数の呼び出しはスキップされます。その結果，無駄な非同期コールを回避することができるというわけです。
