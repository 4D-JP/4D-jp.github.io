...
* ACI0102725 `var`シンタックスを使用して同じ名前の変数を異なるクラスで宣言した場合，シンタックスエラーにもコンパイルエラーも返されませんでした。

* ACI0102710	 `VP SET CELL STYLE`に`Null`を渡した場合，指定したレンジの最初のセルは背景がクリアされ，残りのセルは更新されませんでした。

* ACI0102599 Mac版のみ。4D for iOSのリストフォームまたは詳細フォームにフィールドを追加してから削除し，再び追加した場合，フィールドが表示されませんでした。`defaultField`プロパティが追加されないためです。
