...
* ACI0103791 クライアント/サーバー版のみ。ORDAでアクセスしたBLOBを元よりも小さなサイズのBLOBで上書き保存することができませんでした。元よりもおおきなサイズのBLOBであれば問題ありません。

* ACI0103785 クライアント/サーバー版のみ。サイズが`1`バイト以上`99`バイト以下のBLOBにORDAでアクセスすることができませんでした。`size`プロパティが未定義の`4D.Blob`オブジェクトが返されました。

* ACI0103758 リストボックスを垂直方向にリサイズした場合，*On Display Detail* イベントが過剰に発生しました。

* ACI0103725 リストボックスを垂直方向にリサイズした場合，出現した行で追加の*On Display Detail* イベントが発生しませんでした。垂直スクロールバーを操作してリストボックスを再描画すれば，出現した行の内容が表示されます。

* ACI0103725 ウィンドウを垂直方向にリサイズした場合，リストボックスが完全に再描画されませんでした。
