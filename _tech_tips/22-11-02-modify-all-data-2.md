...
EXPORT STRUCTUREでテキストとして読み込んでいますが、XMLフォーマットなのでDOMコマンドで当該属性値を取り出すことも可能ですが、このサンプルでは正規表現で属性値を切り出しています。
DOMコマンドの場合には属性値が存在しない、つまり入力／出力フォームの設定が無いような場合にエラー処理を行う必要があるため、エラー処理なしに属性値を取り出すために正規表現を利用しました。
