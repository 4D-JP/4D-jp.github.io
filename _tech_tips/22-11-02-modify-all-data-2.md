...
[すべてのテーブルのデータにアクセスする方法（その１）](../22-11-02-modify-all-data/)では、すべてのテーブルのデータにアクセスするためのリストボックスを利用するサンプルを紹介しましたが、今回はすでに作成された既存フォームを使用して表示するサンプルを紹介します。
...
入力／出力フォームの設定は、プロジェクトモードの場合catalog.4DCatalogファイルにテーブルの属性として記述されます。
catalog.4DCatalogの内容を読み出す方法は色々と考えられますが、このサンプルではEXPORT STRUCTUREコマンドを使用しています。

EXPORT STRUCTUREでテキストとして読み込まれたcatalog.4DCatalogの内容は、XMLフォーマットなのでDOMコマンドで当該属性値を取り出すことも可能です。
しかし、このサンプルでは、DOMコマンドではなく正規表現で属性値を文字列として切り出しています。
DOMコマンドの場合には属性値が存在しない、つまり入力／出力フォームの設定が無いような場合にエラー処理を行う必要があります。
このサンプルではエラー処理を簡略化する木手味のために、正規表現を利用しました。

その他の部分につきましては、[すべてのテーブルのデータにアクセスする方法（その１）](../22-11-02-modify-all-data/)を参照してください。
