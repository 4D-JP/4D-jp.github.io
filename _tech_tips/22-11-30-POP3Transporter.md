...
[^id]: idが文字列である理由は、RFCで定められたメッセージ番号をそのまま利用するためです。参考：[RFC1939 UIDLコマンド](https://www.rfc-editor.org/rfc/rfc1939#page-12) 
...
このXMLをコピーして、ストラクチャエディタ上でペーストすると、この例題で使用するテーブルを簡単に作成できます[^StructureEditor]。
