...
４Ｄのフォームオブジェクトにファイルやフォルダーをドラッグ・アンド・ドロップしたとき、４Ｄの自動ドロップに頼ることはできません。
...
また、大量の空のパスが渡されますので、空のパスを扱わないようにする工夫が必要になります。
このサンプルでは、有効なパスをコレクション化することで、扱いをシンプルにすることができました。
