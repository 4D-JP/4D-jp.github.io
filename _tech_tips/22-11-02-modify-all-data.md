...
すべてのテーブルのデータにアクセスするためのメソッドの紹介

<!--more-->

ビルドしたデータベースでは、デザインモードのカレントテーブル表示が利用できません。また、デザインモードのカレントテーブル表示は、既存フォームを利用してデータにアクセスするため、フォームを用意しないと利用することができません。

そこで、すべてのデータにアクセスできるリストボックスを表示するサンプルを作成してみました。

このサンプルは、たった１つのメソッドで完結しています。フォームを用意しなくとも、リストボックスですべてのデータにアクセスできるダイアログウィンドウを表示します。このサンプルのコードを記述したメソッドをビルドしたアプリケーションに、密かに忍ばせておくことで、データのメンテナンスを簡易ながらも行うことができるようになります。
